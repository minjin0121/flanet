...
/* 
데이터 수직, 데이터 분석, 단계별 딥러닝 학습, 딥러닝 모델 커스텀 카테고리를 제공해주고 있습니다. 
          또한 실제 학습과 같은 환경을 구성하기 위해 
          수집 - 전처리 - 학습 - 평가 - 추론으로 블록 단위를 나누어서 학습< */

/* 블록코딩을 어떻게 사용할지 모르겠죠?
          이번 장에서는 ~~입니다.
            Prophet 모델 사용하기
          2단계 : FlaNET 블록 코딩 사용해보기 - 1

          블록 코딩을 어떻게 사용할지 모르겠죠?? ~~입니다!
          머신 러닝과 딥러닝은 수집 - 전처리 - 학습 - 정확도 - ~~로 되어있어요.
          블록 코딩에서도 ~ 로 구성되어 있습니다. \
          ppt 지금까지 제출한 그림으로 해도 조을듯! 간단 원리 설명하고
          1,2,3단계로 나눠서 ~~ 하고 있음
          1단계에서는 prophet 라이브러리와 lstm, ~~
          블럭 움직이는 것을 움짤로?? */

/* 사용자가 처음 웹크롤링을 한다면
어떻게 코드를 짜야하는지도 모르고 여기저기 블로그를 검색하면서 해볼텐데
저희 "실시간 데이터 수집" 블록을 실행해본다면
웹크롤링 코드를 주고 동작하는 결과를 보여줘서 블로그 내용보다는 조금더 시각적으로 웹크롤링을 공부해볼 수 있지 않을까 생각해봤습니다 */

/* <br /> 통해서 다시 학습하거나 분석할 때 사용이 됩니다. 저장을 통해
사용했던 데이터를 데이터가 입력되었다면 학습을 해야합니다! 그러나
학습을 하기 위해서는 모델에 맞게 데이터를 정제해주는 과정이
필요합니다. 데이터 수집, 입력 블록 후, 데이터 정제 블록을 붙여서
사용합니다. */
